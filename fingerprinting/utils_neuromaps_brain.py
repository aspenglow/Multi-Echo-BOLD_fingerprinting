...
#     cmap=my_cmap
    ...
    kws = {'location': 'bottom', 'label_direction': 0, 'decimals': 1,
...
def full_view(current_nodestrength, edges=False, surftype='inflated', xlabel=r'$<s_i> $', q_thresh=0.25, cmap='custom',
    brightness=0.7, exp_form=True, parcellation=100, vmin_plot=None, vmax_plot=None,graymap_rev=False,alpha_graymap=1.):
    '''
...
#                 color_range=(vmin,vmax))
    ...
    fig.savefig('all_lateral.svg', dpi=150)
    plt.close()
    ...
    fig.savefig('all_medial.svg', dpi=150)
    plt.close()
    ...
    if edges == True:
        p.add_layer({'left': lh_regions, 'right': rh_regions}, cmap=mygraymap,
                    as_outline=True, cbar=False)
    fig = p.build(scale=(2, 2))
    fig.savefig('dorsal.svg', dpi=150)
    plt.close()
    ...
    fig = p.build(scale=(2, 2))
    fig.savefig('ventral.svg', dpi=150)
    plt.close()
    ...
    fig = p.build(scale=(2, 2))
    fig.savefig('anterior.svg', dpi=150)
    plt.close()
    ...
    fig = p.build(scale=(2, 2))
    fig.savefig('posterior.svg', dpi=150)
    plt.close()
...
def compose_full_view(remove_files=True, output_name='fig_final', save_svg=False, save_png=False):
    ...
    pl_cb.moveto(97.5, 100, scale_x=scaling_central, scale_y=scaling_central)
    pl_cu.moveto(43.5, -8, scale_x=scaling_central, scale_y=scaling_central)

    pl_u.moveto(-52.5, -16.5, scale_x=scaling_central, scale_y=scaling_central)
    pl_b.moveto(-82, 24, scale_x=scaling_medial, scale_y=scaling_medial)

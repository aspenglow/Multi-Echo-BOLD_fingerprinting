...
for echo_index_front in range(echoes_total_num):
    for echo_index_back in range(echoes_total_num):
        ...
        print("Calculating result with " + str_echo_index_front + "-" + str_echo_index_back + " echo pair.")
        ...
        for subject_index in range(subjects_total_num):
            FC_front = np.identity(FC_side_length)
            FC_front[mask] = recon_matrix_opt_front[:,subject_index]
            FC_front = FC_front.transpose()
            FC_front[mask] = recon_matrix_opt_front[:,subject_index] 
            echoes_FCs_front_recon[echo_index_front][subject_index] += FC_front
            
            FC_recon_back = np.identity(FC_side_length)
            FC_recon_back[mask] = recon_matrix_opt_back[:,subject_index]
            FC_recon_back = FC_recon_back.transpose()
            FC_recon_back[mask] = recon_matrix_opt_back[:,subject_index]
            echoes_FCs_back_recon[echo_index_back][subject_index] += FC_recon_back # Each FC will have overlaps with # of echoes. Need to divide by it in the end.

        ### Draw related results for each echo-pair
        fig = plt.figure(figsize=(18,10), dpi=100)
        font = {'size':20}
        left, bottom, width, height = -0.01, 0.55, 0.35, 0.35
        ax0 = fig.add_axes([left, bottom, width, height])
        c0 = ax0.pcolor(Ident_mat_orig)
        ...
        left, bottom, width, height = -0.01, 0.1, 0.35, 0.35
        ax1 = fig.add_axes([left, bottom, width, height])
        c1 = ax1.pcolor(Ident_mat_recon_opt)
        ...
        left, bottom, width, height = 0.45, 0.15, 0.45, 0.7
        ...
        fig.text(0.58,0.35,"m_star="+str(m_star),fontdict={'size':25})
        fig.text(0.58,0.3,"Idiff_opt=="+str(Idiff_opt)[:6]+"%",fontdict={'size':25})
...
fig, (ax0) = plt.subplots(1, 1, figsize=(18,10))
c = ax0.pcolor(Idiff_matrix)
ax0.set_title('Idiff matrix')
ax0.invert_yaxis()
ax0.set_aspect('equal', adjustable='box')
ax0.set_xlabel('Echo ' + str_echo_index_front + '(front)') 
ax0.set_ylabel('Echo ' + str_echo_index_back + '(back)')
...
recon_FC_root_path = os.path.join(result_path, "reconstructed_FCs")
...
for echo_index in tqdm(range(echoes_total_num), leave=False, desc='echoes'):
    for subject_index in tqdm(range(subjects_total_num), leave=False, desc='subjects'):
        FC_front = echoes_FCs_front[echo_index, subject_index]
        FC_front_recon = echoes_FCs_front_recon[echo_index, subject_index]

        FC_back = echoes_FCs_front[echo_index, subject_index]
        FC_back_recon = echoes_FCs_front_recon[echo_index, subject_index]
        
        str_subject_index = str(subject_index+1)
        str_echo_index = str(echo_index+1)
        # Save FC as .txt
        recon_FC_front_path = os.path.join(recon_FC_root_path, "subject-" + str_subject_index + "_echo" + str_echo_index + "_front.txt")
        np.savetxt(recon_FC_front_path, FC_front_recon)
        recon_FC_back_path = os.path.join(recon_FC_root_path, "subject-" + str_subject_index + "_echo" + str_echo_index + "_back.txt")
        np.savetxt(recon_FC_back_path, FC_back_recon)
        # Save FC as .jpg
        fig , ax = plt.subplots(1,2, figsize=(18,10))
        ax0 = ax[0]
        c0 = ax0.pcolor(FC_front)
        ax0.set_title('Original FC')
        ax0.invert_yaxis()
        ax0.set_aspect('equal', adjustable='box')
        ax0.set_xticks([])
        ax0.set_yticks([])
        ...
        ax1 = ax[1]
        c1 = ax1.pcolor(FC_front_recon)
        ax1.set_title('Reconstructed FC')
        ax1.invert_yaxis()
        ax1.set_aspect('equal', adjustable='box')
        ax1.set_xticks([])
        ax1.set_yticks([])
        ...
        recon_FC_front_img_path = os.path.join(recon_FC_root_img_path, "subject-" + str_subject_index + "_echo" + str_echo_index + "_front.jpg")
        plt.savefig(recon_FC_front_img_path)

        c0 = ax0.pcolor(FC_back)
        c1 = ax1.pcolor(FC_back_recon)

        recon_FC_back_img_path = os.path.join(recon_FC_root_img_path, "subject-" + str_subject_index + "_echo" + str_echo_index + "_back.jpg")
...
for echo_index in range(echoes_total_num):
    ...
    fig, ax = plt.subplots(1, 2, figsize=(18,10))
    ax0 = ax[0]
    FC_orig = echoes_FCs_front_avg[echo_index]
    FC_recon = echoes_FCs_front_recon_avg[echo_index]
    c = ax0.pcolor(FC_orig)
    ...
    FC_avg_path = os.path.join(FC_avg_img_path, "FC_echo_" + str(echo_index+1) + ".jpg")
    plt.savefig(FC_avg_path)
    plt.close()
